...
public class Painter extends JPanel implements ActionListener{
	...
	double hiplength;
	double scale;
	public static double[][] CoMx,CoMy;
	public static double[][] angle;
	Timer tm;
	int start, rightmaxspeed, leftmaxspeed, stancecount, leftcount, rightcount, stancecounttot=0, leftcounttot=0, rightcounttot=0;
	public static int[] stance, leftswing, rightswing;
	public static double maxllength;
	
	public Painter(int[] LED, int freq){
		...
		try
		{
		br=new BufferedReader(new FileReader("/Users/akhil/Documents/marker.txt"));
		...
		for(int i=0;i<TotalFrames;i++)
		{
			...
			if(start==1&i>1)
			{
					if(x[i][2]<x[i-1][2]+3&x[i][8]<x[i-1][8]+3&stancecount==1)
					{
					...
					if(x[i][2]>x[i-1][2]+3&x[i][8]<x[i-1][8]+3&rightcount==1)
					{
					...
					if(x[i][2]<x[i-1][2]+3&x[i][8]>x[i-1][8]+3&leftcount==1)
					{
